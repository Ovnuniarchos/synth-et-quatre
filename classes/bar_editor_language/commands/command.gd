...
func find_invalid_tokens(tokens:Array,cmd_data:Array,from:int)->int:
	...
	for i in types.size():
		if types[i]==BEConstants.TOKEN_OPTIONAL:
			if from>=tokens.size() or tokens[from][BEConstants.TK_TYPE]==BEConstants.TOKEN_WHITESPACE:
				return -1
			continue
		if from>=tokens.size():
			return tokens.size()
		if tokens[from][BEConstants.TK_TYPE]!=types[i]:
			return from
		from+=1
	return -1
...
func common_checks(tokens:Array,from:int,is_cmd:bool,expected_cmd:bool,command:Array)->LanguageResult:
	if is_cmd!=expected_cmd:
		return LanguageResult.new(LanguageResult.ERR_UNEXPECTED_TOKEN,{"s_token":token_error_string(tokens[from]),"i_start":tokens[from][BEConstants.TK_START]})
	var end:int=tokens[-1][BEConstants.TK_END]+1 if tokens[-1][BEConstants.TK_END]>-1 else -1
	if from+command[BEConstants.CMD_MIN_TOKENS]>tokens.size():
	...
	if itoken_pos==tokens.size():
		return LanguageResult.new(LanguageResult.ERR_MISSING_PARAMS,{"i_start":tokens[from][BEConstants.TK_START],"i_end":-1})
	elif itoken_pos>-1:
