...
func calculate()->Array:
	...
	for i in 4:
		calculate_option_slot(
			...
			TriangleNodeConstants.TRQ_MAX,
			quarters[i]
		)
	NODES.triangle(output,max(1.0,size*range_length),fposmod(range_from*size,size),
...
func duplicate(container:Reference)->WaveNodeComponent:
	var nc:TriangleNodeComponent=.duplicate(container) as TriangleNodeComponent
