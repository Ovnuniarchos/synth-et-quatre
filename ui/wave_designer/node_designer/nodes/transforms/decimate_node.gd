...
func _ready()->void:
	var ops:Array=[]
	for i in DecimateNodeConstants.LERP_END:
		ops.append({"label":DecimateNodeConstants.LERP_STRINGS[i],"id":i})
...
func set_parameters()->void:
	if not is_node_ready():
		yield(self,"ready")
	$Samples.set_value(node.samples_value)
...
func _on_Samples_value_changed(value:float)->void:
	node.samples_value=value
	emit_signal("params_changed",self)
