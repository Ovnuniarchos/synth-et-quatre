...
func calculate()->Array:
	...
	for i in 4:
		calculate_option_slot(
			...
			SawNodeConstants.SWQ_MAX,
			quarters[i]
		)
	NODES.saw(output,max(1.0,size*range_length),fposmod(range_from*size,size),
...
func duplicate(container:Reference)->WaveNodeComponent:
	var nc:SawNodeComponent=.duplicate(container) as SawNodeComponent
